...
const createGroup = async (req, res) => {
    const {userId, groupType, groupLicence, groupName, groupContact, groupEmail, groupArea} = req.body;
    try{
        ...
        if (existingGroup){
            ...
            return res.status(400).json({ error: `${req.body.groupLicence} is already registered!` });
        }

        const group = await Group.create({groupType, groupLicence, groupName, groupContact, groupEmail, groupArea});
        
        const user = await User.findByIdAndUpdate(userId, {group: group._id}, {new: true});
...
const updateGroup = async (req, res) => {
    const {_id, groupType, groupLicence, oldGroupLicence, groupName, groupContact, groupEmail, groupArea} = req.body;
    if (!mongoose.Types.ObjectId.isValid(_id)) {
        return res.status(400).json({ error: 'Not valid group ID' });
    }

    try{
        //if group licence is changed
        if (groupLicence !== oldGroupLicence){
            const existingGroup = await Group.findOne({groupLicence:groupLicence});

            if (existingGroup){
                createError.BadRequest(`${groupLicence} is already registered!`);
                return res.status(400).json({ error: `${groupLicence} is already registered!` });
            }
        }
