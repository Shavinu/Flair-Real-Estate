...
const Test = () => {
  ...
  const [projectPriceRange, setProjectPriceRange] = useState({ minPrice: initialData.projectPriceRange[0].minPrice, maxPrice: initialData.projectPriceRange[0].maxPrice });
  const [projectDescription, setProjectDescription] = useState(initialData.projectDescription);
  const [userGroup, setUserGroup] = useState();
  const [editableBy, setEditableBy] = useState(initialData.editableBy);
  const [commissionData, setCommissionData] = useState(initialData.projectCommission);
  const [projectLocation, setProjectLocation] = useState(initialData.projectLocation);
  const [coordinates, setCoordinates] = useState({ lat: initialData.projectLocation[0].latitude, lng: initialData.projectLocation[0].longitude });
  // const [initialProjectLocation, setInitialProjectLocation] = useState(initialData.projectLocation);
